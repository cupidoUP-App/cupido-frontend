/** Página de restablecimiento de contraseña. Lee el token de la URL (path o query param) y envía la nueva contraseña al backend. */
...
const ResetPasswordPage: React.FC = () => {
  ...
  /** Estado: indica si se está enviando la solicitud. */
  const [isSubmitting, setIsSubmitting] = useState(false);
  /** Estado: token de recuperación extraído de la URL. */
  const [token, setToken] = useState<string | null>(null);

  const { toast } = useToast();

  /** Extrae el token de la URL (path param o query string) al montar el componente. */
  useEffect(() => {
    setNewPassword('');
    setConfirmPassword('');
    const paramToken = params.token;
    const queryToken = searchParams.get('token');
    const foundToken = paramToken || queryToken;
    if (foundToken) setToken(foundToken.trim());
  }, [params.token, searchParams]);

  /** Valida campos y envía la nueva contraseña con el token al backend. */
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await authAPI.resetPasswordConfirm({ token: token || '', nueva_contrasena: newPassword });
      ...
      if (error.response?.data) {
        ...
        if (data.nueva_contrasena) errorMessage = data.nueva_contrasena[0] || 'La nueva contraseña no cumple con los requisitos.';
